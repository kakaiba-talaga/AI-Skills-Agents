...
| **Feedback loops and self-correction** | Verification failures trigger a fix cycle (executor → verifier, up to 3 loops). Agent failures trigger debugger diagnosis, then model escalation (sonnet → opus, opus → fable), before reaching user escalation. |
...
- **3rd attempt fails**: Escalates model one tier up (sonnet → opus, opus → fable) with full error history. Skipped if already on fable (or already on opus for `security-reviewer`), or if the failure is a blocker or scope issue.
...
The team manager retries a failing task up to 4 times (sonnet × 2 → debugger diagnosis → one-tier-up escalation → user escalation, or 3 times if already on fable). If you see the same task cycling, type `stop` to halt dispatching, then inspect what the last agent returned. You can then `skip` the task, adjust the plan, and resume.
