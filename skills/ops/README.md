...
1. **Intake** — Receives a spec, requirement, or existing plan. If a ClickUp task ID is referenced, pulls task details through the `/clickup` skill, skipping enrichment if it is unavailable.
...
> **Note:** When `/ops` is wrapped with `/ralph-loop` (`/ops ralph`), nested-skill invocations inside the ralph-loop wrapper (e.g., ralph-loop's Cleanup stage → `/deslop`) are governed by ralph-loop's own prompt. `skills/ralph-loop/cleanup-deslop.md` now carries its own quiescence precondition before that stage's deslop escalation, requiring every subagent dispatched during the iteration (including an enclosing `/ops ralph` cycle's own parallel task dispatches) to have returned first, matching the requirement above.
...
- **Model escalation** — if an agent fails twice, the 3rd attempt escalates one tier up (sonnet → opus, opus → fable) before reaching user escalation. Extends the failure handling from 3 strikes to 4 (sonnet → sonnet+context → opus → user), or 3 strikes to 4 (opus → opus+context → fable → user). Two carve-outs: `security-reviewer` caps at opus (Fable 5's cybersecurity safety classifiers can refuse security-analysis content mid-run); an agent already assigned fable skips the 3rd-attempt escalation and goes straight to user escalation (a 3-attempt ceiling rather than 4). The `opus → fable` step now sits behind a confirmation gate — interactive (and `--supervised`) runs wait for the user; autonomous runs wait best-effort ~1 minute, then default NO and continue the 3rd attempt on the original `opus` model rather than spending `fable`.
...
| `orchestrator-obligations.md` | Board-versus-artifact authority (the board is the only dispatch-state file; a user-facing tracking artifact is never a substitute for it), enrolling work as it arrives mid-run and closing out every task that enrolment opens, checking the change an agent asserts before relaying its claim, naming the observation behind a self-made claim rather than its provenance and citing a same-turn command against the artifact before recommending a revert, delete, or skip, dispositioning a finding an agent reported but did not fix, committing a completed task's diff and staging it by what git already tracks, quoting the user's own words for any constraint attributed to them, naming the failing clause before overriding the triage gate's answer, and the turn-end check on a turn that named its own next action | Any agent spawn; an agent return; before ending any turn (MUST) |
