...
| Outcome | Action |
| :--- | :--- |
| **Passed** — acceptance criteria met | Same `Edit`: `status` → `"completed"`, increment `attempts`. **Cursor:** Edit → verify → `TodoWrite`, then write handoff (see Handoff Documents). Check for newly unblocked tasks. |
