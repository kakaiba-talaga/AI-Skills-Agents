...
- **No reply by the deadline → default NO:** clear `pending_fable_confirm`, run the 3rd attempt on the recorded original `opus` model, and keep the run unblocked. The Verify → Fix loop cap still applies — in autonomous mode, rungs 4 and 5 re-dispatch on the opus model before escalating to the user at the 5-loop cap.
