...
**Step 4 — Process results.** When an agent returns, **immediately** update the state file: record `completed_at` with ISO-8601 timestamp, calculate and store `duration_seconds`, increment `attempts`. For a detached dispatch, an agent's return is the moment the orchestrator processes its completion notification — `completed_at` is recorded then, not when the agent itself finished working. Pinning the return event to notification-processing keeps this immediate: there is a definite moment the return happens, and the timestamp is written at that moment, so Non-negotiable #3 still holds. Read `duration_seconds` from the duration the completion notification itself reports — the agent's own measured runtime — rather than computing it by subtracting `started_at` from `completed_at`: a notification can sit unprocessed for a while before the orchestrator picks it up, and subtracting would fold that pickup latency into the recorded duration, silently inflating every measurement and corrupting the estimates future runs calibrate against. See `skills/ops/timing-edge-cases.md` for the rule covering background notification pickup latency. Write the state file to disk. **Cursor only:** after Write, Read-verify per § **Cursor: state file sync**, then update `TodoWrite` for any `status` change in the outcome table below. `TodoWrite` alone does not satisfy Non-negotiable #3. (Non-negotiable — see #3.) When two or more agents' completion notifications land in the same orchestrator turn, this step still runs once per return, in arrival order, each with its own state-file write — never one batched write for the whole turn: the one-notification procedure above applies unchanged to every return in that sequence, so a second return's `completed_at` is never recorded against a different task's return. Each per-return write also fires the state cache's "Task completed" invalidation event separately, once per completion rather than once for the turn — see `skills/ops/state-schema.md`, § State Cache Semantics, the second entry in its invalidation-events list — so the cached board snapshot is re-read after every completion, since each write just mutated it.
