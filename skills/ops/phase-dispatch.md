...
- **No reply by the deadline → default NO:** clear `pending_fable_confirm`, run the 3rd attempt on the recorded original `opus` model, and keep the run unblocked. The "4th attempt → escalate to user" backstop still applies if that opus retry also fails.
