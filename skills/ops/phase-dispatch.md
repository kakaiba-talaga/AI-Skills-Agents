...
The board already exists at this point, so this dispatch enrols: an internal bookkeeping task moved to `in_progress` in the message that carries the spawn (Non-negotiable #13), closed out to a terminal status when the preflight returns and before the first Phase 3 dispatch goes out. A run that stops here on a critical failure still closes the row first, so the board it leaves behind holds nothing that reads as running (`orchestrator-obligations.md` § Closing out what you enrolled).
...
**Forbidden:** Calling `TodoWrite` for a status change without completing **Edit → Read verify** on the board file in the **same assistant turn** first.

**Required ritual** (every status change: `pending` → `in_progress`, completion, failure, new task, cancel, adaptation) — the **Edit → Read verify** ritual:

1. `Read` the board file fresh from disk, unless this turn already read or wrote it and it is still cached. `Edit` requires the file to have been read (or written) in the current session, and its anchor is byte-exact — the state cache is a semantic snapshot with no byte-exactness guarantee, so it can never itself serve as the anchor source. See `state-schema.md` § Anchor derivation for targeted `Edit`.
2. Compose a single `old_string` / `new_string` pair covering every field that changes together for this mutation (`status`, `started_at`, `completed_at`, `duration_seconds`, `attempts`, etc.) — never split co-changing fields across separate `Edit` calls.
3. Apply the targeted `Edit` to `.ops-state/<run-id>-board.json`.
4. `Read` the file back; confirm the mutated field(s) match AND that the document still parses as valid JSON. If either check fails, apply a corrective `Edit` before proceeding — **do not** call `TodoWrite` until verify passes. `Edit` fails loudly on a bad anchor, but a malformed replacement string lands silently, which is why the JSON-parse check is not optional.
...
This is the core orchestration loop. Repeat until every task has reached a terminal status (`completed`, `failed`, `blocked`, `deleted`, or `cancelled`) or the user intervenes:
...
This predicate does not separately check `agent_type`, because every documented task-creation path already requires it: `phase-intake.md`'s task-shape section lists `agent_type` as a required field on every new task, and the mid-run `add` command and the re-plan board rewrite (Step 5b's Board-rewrite invariants) both produce tasks through that same shape. A `pending` task without `agent_type` would mean a creation path silently skipped that requirement, or the board was edited out-of-band — the latter already an unsupported state (`state-schema.md`'s Safety note). Re-checking a field the write side already guarantees would only defend against a write-side bug this predicate cannot itself fix. If a future creation path is ever added outside the documented task-shape convention, the guard belongs there, at the point that path writes the task, not here.
...
- **User mid-run command** (`add`, `drop`, `reprioritize`, `do #N next`, `skip`) — re-read after processing the command. A message carrying none of these is a conversational beat, not this event: it triggers no re-read.
...
Between these events, operate on the cached snapshot **for scanning** — deciding which tasks are ready, rendering a dashboard. Do not re-read for that purpose on routine Step 1 → Step 2 → Step 3 cycles within one dispatch iteration. This carve-out never extends to a mutating `Edit`: per `state-schema.md` § Anchor derivation for targeted `Edit`, the cache has no byte-exactness guarantee and can never serve as the source of an `Edit` anchor — every mutating `Edit` requires a fresh `Read` immediately beforehand, regardless of cache state.
...
**Step 2 — Batch parallel work.** Dispatch tasks on different files/modules concurrently, up to `--parallel N`. The cap used to hold automatically under blocking dispatch: the orchestrator waited for every dispatched task to return before this step ran again, so nothing was ever in flight when the next batch was counted. Background dispatch breaks that — the orchestrator regains its turn immediately and can reach Step 1 again before a prior batch returns (see Step 6) — so the cap must now be enforced explicitly rather than by blocking. `N` counts tasks with `status == "in_progress"` right now, not the size of the batch being assembled: dispatch only enough new tasks to bring that in-progress count up to `N`.

Never parallelize a task with any **in-flight sibling** task that touches the same file (`SKILL.md`, § Parallel Safety Rules), checked against every task currently in progress, not only the batch under assembly — two tasks in separate batches can collide on a file exactly as two tasks in the same batch can. When in doubt, run sequentially. The same holds for two merge dispatches that target the same branch — already barred by the sequential-merge requirement in `agents/git-master.md`'s "Branch workflow" section and the same-branch prohibition in `SKILL.md`, § Parallel Safety Rules — so sequence them with a `blocked_by` edge between the merge tasks, the same mechanism `SKILL.md`, § Internal Tasks already makes available since merge branches are trackable as board tasks.

**Step 3 — Dispatch agents.** For each task (or parallel batch):

1. **Read the state file from disk** — a fresh `Read`, not the cached snapshot; the cache has no byte-exactness guarantee and can never serve as an `Edit` anchor's source (`state-schema.md` § Anchor derivation for targeted `Edit`). Then apply a targeted `Edit` to the state file: set `status` to `"in_progress"`, record `started_at` with ISO-8601 timestamp, record `model_used`, and clear `completed_at` back to `null` — a re-dispatched task must not carry a stale completion timestamp from a prior attempt. Fold all four fields into one `old_string` / `new_string` replacement. `Read` the file back to verify. **Cursor only:** follow § **Cursor: state file sync** (Edit → Read verify → then `TodoWrite`) in the same turn before step 2.
...
**Status–spawn atomicity (Non-negotiable #11).** Item 1 (the `in_progress` state-file `Edit`) and items 4–5 (the `Agent()` spawn[s]) MUST be emitted in the **same assistant message** — never split across turns. Do not write `in_progress`, and do not report a task as running in a dashboard or in prose, in any message that does not also contain its spawn. For a parallel batch, one `Edit` per task rides the same message as all the batch's `Agent()` calls — never one `Edit` spanning several tasks' fields, which either fails on a non-unique anchor or reaches across unrelated fields. A status written without a spawn beside it is a phantom `in_progress` that `resume` and `status` will later treat as a live-but-orphaned dispatch. Re-displaying an already-running task in a later turn is not a new transition — that is fine. (Mechanically: emit the state-file `Edit` and the Agent tool call(s) as parallel tool uses within one assistant message.)

**Next-turn reconciliation check (mandatory).** A dispatch message's transition `Edit` can fail on a bad anchor after its `Agent()` spawn has already gone out, leaving a live agent whose task still reads `pending`. On the very next turn — the turn processing that dispatch message's tool results — confirm that every transition `Edit` it contained reported success. If one did not, and the spawn it was paired with did go out, apply the reconciliation write described in `SKILL.md`'s Status–spawn atomicity carve-out before Step 1 advances the dispatch loop again.

**Two kinds of state-file writes.** *Bookkeeping* writes record something that already happened or adjust the board — completions, timing, adaptations, dependency edges, notes — and may ride any message, including one that spawns nothing. *Transition* writes move a task to `in_progress`, and may ride only a message that also carries that task's spawn. The two are mechanically identical edits to the same file, which is why a transition write is easy to fold into ordinary bookkeeping without noticing the difference. Work backed by a `Skill()` call rather than an agent spawn is bookkeeping of this kind: the pass row is recorded on the board retroactively, as a completed row, and is never transitioned through a running state, because the `Skill()` call produces no spawn for that row to represent. That reaches the pass row and nothing further. A nested skill runs inline in this context, so every `Agent()` or `Task()` call it makes is this orchestrator's own spawn, and each one enrols on its own task, transitioned in the message that carries the call, exactly as Non-negotiable #13 requires. Where the harness invokes the nested pass itself through a spawn tool rather than through `Skill()`, the pass row is a spawn too and transitions like any other dispatch. Which of the two applies is settled by reading back the tool call the turn actually emitted, never by how the work is classified.

When a turn both records a completion and dispatches the next task, two orderings satisfy the atomicity requirement above: the completion write, the transition write, and the spawn all in one message, or the completion write alone in one message followed by the transition write and the spawn together in the next. The order that looks tidiest — grouping both writes together and spawning only afterward — is not one of them. A parallel batch follows the same discipline taken to its limit: one transition write per task, and every spawn in the batch, ride one message. A transition-write message followed by separate spawn messages is invalid even when every spawn eventually goes out, because the board is wrong for the interval between the write and the last spawn, and would stay wrong had the turn ended there.

**Before sending any message that writes the state file, ask:** does this write set a task to `in_progress`, and if so, does this same message also carry that task's spawn? A yes followed by a no means the message is malformed — restructure it before sending.
...
Default dispatch is **background**; the closed foreground list lives in the companion.
...
**Nested skill invocations:** When the team manager invokes a nested skill (e.g., `/deslop`, `/cross-memory reflect`) during the dispatch loop, execute the write-before / clear-after ritual to prevent the turn from ending on the nested skill's return. A nested skill may be entered while background work is still in flight; an arriving completion is processed at the next beat as usual. The ritual has eleven steps (5 write-before + 6 clear-after):

- **Write-before** (immediately before the nested-skill call): (1) build the `pending_nested_skill` record with fields `skill`, `invoked_at`, `resume_phase`, `resume_notes`; (2) read the state file from disk; (3) apply a targeted `Edit` setting the `pending_nested_skill` field on the root object to the record from step 1; (4) `Read` the file back to confirm the field matches and the document still parses as valid JSON; (5) issue the nested-skill call.
- **Clear-after** (immediately after the nested skill returns, in the same turn): (1) read the state file from disk (cache was invalidated — see Step 1); (2) read `pending_nested_skill.resume_phase` and `resume_notes` to identify where to resume and how to proceed; (3) capture any output the nested skill produced that downstream phases need — write it into a handoff file where one exists, or hold it in-turn for the next agent's brief when no handoff procedure applies; (4) apply a targeted `Edit` clearing `pending_nested_skill` back to `null`; (5) `Read` the file back to confirm the field is `null` and the document still parses as valid JSON; (6) execute the `resume_phase`-specified next action. **Do not end the turn.** See Non-negotiable #10.

**Step 4 — Process results.** When an agent returns, **read the state file from disk** — a fresh `Read`, not the cached snapshot; the cache has no byte-exactness guarantee and can never serve as an `Edit` anchor's source (`state-schema.md` § Anchor derivation for targeted `Edit`) — then **immediately** apply a targeted `Edit` to the state file recording `completed_at` with ISO-8601 timestamp and calculating and storing `duration_seconds`. For a detached dispatch, an agent's return is the moment the orchestrator processes its completion notification — `completed_at` is recorded then, not when the agent itself finished working. Pinning the return event to notification-processing keeps this immediate: there is a definite moment the return happens, and the timestamp is written at that moment, so Non-negotiable #3 still holds. Read `duration_seconds` from the duration the completion notification itself reports — the agent's own measured runtime — rather than computing it by subtracting `started_at` from `completed_at`: a notification can sit unprocessed for a while before the orchestrator picks it up, and subtracting would fold that pickup latency into the recorded duration, silently inflating every measurement and corrupting the estimates future runs calibrate against. See `skills/ops/timing-edge-cases.md` for the rule covering background notification pickup latency. The outcome table below folds `status` and the `attempts` increment into this same `Edit` — every row assigns a `status` so no returned task is left reading `in_progress` on disk, and the `attempts` increment moves per-row so the standing NEEDS_CLARIFICATION exemption (see below) is no longer in tension with an unconditional increment here. See `skills/ops/state-schema.md` § Status enum for the full set of `status` values a task can carry. **Cursor only:** after the `Edit`, Read-verify per § **Cursor: state file sync**, then update `TodoWrite` for any `status` change in the outcome table below. `TodoWrite` alone does not satisfy Non-negotiable #3. (Non-negotiable — see #3.) When two or more agents' completion notifications land in the same orchestrator turn, this step still runs once per return, in arrival order, each with its own targeted `Edit` — never one batched edit for the whole turn: the one-notification procedure above applies unchanged to every return in that sequence, so a second return's `completed_at` is never recorded against a different task's return. Each per-return edit also fires the state cache's "Task completed" invalidation event separately, once per completion rather than once for the turn — see `skills/ops/state-schema.md`, § State Cache Semantics, the second entry in its invalidation-events list — so the cached board snapshot is re-read after every completion, since each edit just mutated it.
...
| Outcome | Action |
| :--- | :--- |
| **Passed** — acceptance criteria met | Same `Edit`: `status` → `"completed"`, increment `attempts`. **Cursor:** Edit → verify → `TodoWrite`, then write handoff (see Handoff Documents). Check for newly unblocked tasks. When this return leaves the task `completed` and its files carry a diff, enrol and dispatch its commit task (see `orchestrator-obligations.md`). |
| **Failed — 1st attempt** | Same `Edit`: `status` → `"pending"` (queued for re-dispatch, not running), increment `attempts`. Re-dispatch with the error appended to the brief. Narrow the scope or add constraints based on what went wrong. |
| **Failed — 2nd attempt** | Same `Edit`: `status` → `"pending"`, increment `attempts`. Dispatch a **debugger** agent (or **debugger-build** if the failure is a build/import/type error) to diagnose the root cause. Use its findings to re-brief the original agent with a corrected approach. Until the debugger returns, the task is not ready — Step 1 must not treat it as a dispatch candidate, however many scans occur while the diagnostic is in flight, so a detached return to Step 1 cannot fire the third attempt blind, before the findings it depends on have arrived. |
| **Failed — 3rd attempt** | Same `Edit`: `status` → `"pending"`, increment `attempts`. Escalate model (sonnet → opus, opus → fable) and re-dispatch with full error history. Skip if already on fable; security-reviewer caps at opus and never escalates to fable. See Model Escalation in Adaptability. The `opus → fable` step goes through the **`fable`-escalation confirmation gate** (see Model Escalation in Adaptability): interactive (and `--supervised`) waits for the user; autonomous waits best-effort ~1 minute then defaults NO and runs the 3rd attempt on the original `opus` model. `security-reviewer` never reaches the gate. |
| **Failed — at the loop cap** | Same `Edit`: `status` → `"failed"`, increment `attempts`. Escalate to the user with: the task, all attempts, errors, debugger findings, and your diagnosis. Pause this chain; continue other independent chains. Cap is mode-aware: 3 loops in interactive/supervised, 5 loops in autonomous. In autonomous mode, loops 4 and 5 re-dispatch on the already-escalated model (opus after the fable gate defaults NO) with debugger findings from the 2nd attempt carried forward — the model tier does not climb further and the fable gate does not re-fire. |
| **Blocked** — agent hit an external dependency or environment issue | Same `Edit`: `status` → `"blocked"`, increment `attempts`. Create a new blocker task describing the issue. Pause dependent chain. Flag to user. |
| **Scope issue** — agent says the plan is wrong or incomplete | Same `Edit`: `status` → `"blocked"`, increment `attempts`. Pause chain. Ask the user whether to re-plan or adjust. |
| **NEEDS_CLARIFICATION** — brief is well-formed but agent has one clarifying question before starting | Same `Edit`: `status` → `"pending"`. Do **not** increment `attempts` (see handling below). See handling below. |

**Findings the agent reported but did not fix.** The outcome table above dispositions the returning task. It says nothing about anything else the agent found, and a return can carry both: acceptance criteria met, plus a defect the agent correctly stayed out of. Those findings do not disposition themselves by being repeated to the user. Each one is queued, deferred onto a card, rejected with a reason, or escalated as a decision that is the user's to make, before this step is done. See `orchestrator-obligations.md`.
...
**Why N = 2 needs no counter field.** Nothing on the state file tracks how many prior check-ins observed a task past the 2.5× threshold, and nothing needs to: within one dispatch attempt, elapsed time only grows, so once a check-in observes the crossing, every later check-in in the same session observes it too. "N = 2 consecutive check-in events" therefore reduces to "this is not the first check-in that has seen this task past the threshold" — a debounce the orchestrator already carries in its own turn history for the session, the same in-session memory the "Warnings are emitted once per threshold crossing per task" rule above already relies on. No state-file field is required to know that. The one place this in-session memory would go missing is a session boundary, and a session boundary is already covered independently: `phase-intake.md`'s `resume` row dispatches a `work-verifier` against every remaining in-progress task unconditionally, without waiting on this trigger at all. This would become reachable only if that unconditional resume-time dispatch were ever weakened to depend on the sustained-`OVERRUN` trigger, or if a single session ran long enough that the harness's own context management discarded a task's prior-observation turns while the task was still the same in-progress dispatch — neither holds today.

The trigger is a **prompt to diagnose, not a decision to act** — firing it dispatches the read-only `work-verifier` and does not by itself re-dispatch or mutate anything. It is a **third, distinct number**, not to be conflated with: the **2.5× display threshold** (see Step 4 — Process results, above, which lights `🔴 OVERRUN`; a single instantaneous 2.5× crossing never fires this sub-step, and `⚠️ SLOW` at 1.5× never does); nor the **orphan-suspicion timeout** `MIN(agent-type default, 3× task estimate)` (`work-verifier.md`, § Orphan detection, which drives `👻 ORPHAN?`), which is usually below 3× (for example, an executor on a 15-min estimate → `MIN(15, 45) = 15 = 1×`). The higher multiple is deliberately derived **from the 2.5× display threshold, not the orphan timeout** — anchoring to the orphan timeout would fire *after* the dashboard already showed `👻 ORPHAN?`, an inverted ordering.

The trigger is evaluated at **every** check-in event (after foreground returns, after background completion notifications, on the turn that responds to a user message, per `dispatch-policy.md`'s Interaction with Health Monitoring section) — not only after a foreground return.

**Diagnosis: dispatch the read-only `work-verifier`.** When the sustained-`OVERRUN` trigger fires, the orchestrator dispatches the read-only `work-verifier` to inspect the suspected-orphan's *work-state*. The dispatch uses the **existing mechanical-agent dispatch shape**: no memory injection (`work-verifier` is in `MECHANICAL_AGENTS`, defined in Step 3 item 3's Mechanical agents paragraph and applied in its Predicate decision tree, both above), with a standard `## Task` / `## Scope` / `## Constraints` brief and orphan detection enabled (the same enablement the `status` row wires in `phase-intake.md`, § Phase 1 — Intake and task board). The dispatch is read-only — `work-verifier` inspects files and git state and reports findings; it modifies nothing and re-dispatches nothing (see `work-verifier.md`'s opening role statement, and its § Lane boundaries). Because the diagnosis is read-only, it requires **no pre-approval in any mode**. The `work-verifier` returns a *work-state* verdict — `completed` / `partial` / `not-started` / `broken` (`work-verifier.md`, § Verdicts) — which describes how much work landed. It does **not** return process liveness; that is outside its lane (`work-verifier.md`, § Lane boundaries) and is supplied separately by the orchestrator's own dispatch bookkeeping.

**Verdict branch and liveness gate.** The sub-step branches on the `work-verifier` work-state verdict, but only after the orchestrator resolves its own liveness determination for the task's agent. Liveness is three-valued, and the only fact the orchestrator can genuinely test about itself — **spawn ownership**, whether it still holds the agent's spawn in its current context — is the whole test:

| Liveness | Determined by | What may act on it |
| :--- | :--- | :--- |
| **live** | the orchestrator holds this spawn in its current context and no completion notification has arrived | Keep watching. **Never re-dispatch.** |
| **finished-unprocessed** | a completion notification has arrived and Step 4 has not yet processed it | Process the completion at this beat. |
| **unknown** | the orchestrator does not hold this spawn in its current context | Dispatch the read-only checker and act on its work-state verdict. Never assume live; never assume dead. |

**The hard gate: the re-dispatch leg requires liveness `unknown`.** In-session, a spawn the orchestrator still holds classifies `live` and is never re-dispatched, no matter how far past the overrun threshold it runs — this is the same "no completion received" signal the `👻 ORPHAN?` flag uses (see `phase-completion.md`, § Team Manager — Status). This makes the duplicate-work failure structurally impossible in-session, not merely unlikely.

**Cross-session, this gate is weaker by design.** The orchestrator holds no spawn carried over from a previous session, so every in-progress task inherited across a session boundary classifies `unknown` — it always routes to the read-only checker, and a `partial` verdict there does permit re-dispatch. That is re-dispatch on evidence, not the impossible re-dispatch the in-session gate guarantees. Do not read the in-session "never re-dispatch a live spawn" guarantee as if it also held across a session boundary — it does not, and treating it as if it did would recreate exactly the duplicate-work risk this gate exists to close.

- **`completed`** — the agent finished and the orchestrator missed the notification. Mark the task done in the state file and write any missing handoff (`work-verifier.md`, § Handoff, the `completed` row). **No re-dispatch.** Append one `type: health-action` entry to `adaptations` with `action_taken: diagnosed-alive`.
- **`not-started` or `partial` AND liveness is `unknown`** — confirmed orphan; unreachable while the orchestrator still holds the original spawn. Enter the recovery path below. Append one `type: health-action` entry to `adaptations` with `action_taken: re-dispatched` (or `action_taken: re-dispatch-escalated` if the recovery escalates).
- **`not-started` or `partial` but liveness is `live` or `finished-unprocessed`** — the agent is slow but alive; a `partial` result reflects how much work has landed, not whether the writer is gone. Keep watching. **No re-dispatch.** Append one `type: health-action` entry to `adaptations` with `action_taken: diagnosed-alive`.
- **`broken`** — a wrong-output event, not a liveness event; the re-dispatch it leads to is unreachable while the orchestrator still holds the original spawn. Route through the existing rollback-then-re-dispatch path (`work-verifier.md`, § Handoff, the `broken` row; `SKILL.md`, § Failure Handling, rollback reference): dispatch a **rollback** agent in the foreground to revert the broken output — before it starts, either zero other tasks are `in_progress`, or the rollback's declared scope is verified file-disjoint from every in-flight sibling — then re-dispatch the original agent on a clean slate. Append one `type: health-action` entry to `adaptations` with `action_taken: re-dispatched`.

**Recovery rule (confirmed orphan).** A confirmed-orphan recovery is governed by **exactly one** rule: the Failure Handling row "Agent timeout or crash → Retry once with same brief, then escalate" (`SKILL.md`, § Failure Handling) — one same-brief re-dispatch, then escalate to the user if that also orphans or fails (logging `action_taken: re-dispatch-escalated`). The confirmed orphan does **not** enter the four-step Step-4 outcome ladder above and no debugger is interposed — a debugger diagnoses output, build, or logic failures, whereas an orphan is a liveness event the `work-verifier` plus the orchestrator's own signal have already diagnosed. The retry cap stays single-sourced in `SKILL.md`, § Failure Handling, by cross-reference; it is not restated here.

The re-dispatched orphan inherits the **same foreground/background and worktree posture** as the original dispatch: a background-with-worktree orphan is re-dispatched background-with-worktree, reusing the surviving worktree if `work-verifier` reports it intact (`work-verifier.md`, § Worktree tasks), else falling back to the main tree. The orphan re-dispatch **inherits the `attempts` counter** — it continues the same task's attempt count rather than resetting it. The re-dispatched orphan is **itself health-monitored** under the same sustained-`OVERRUN` trigger — no exemption. The recursion is bounded by the single-retry `attempts` cap from `SKILL.md`, § Failure Handling: at most one orphan re-dispatch per task; a re-dispatched orphan that also orphans escalates to the user rather than triggering another re-dispatch.

**Parallel safety on re-dispatch.** Before re-dispatching a confirmed orphan, the orchestrator applies the **existing** Parallel Safety Rules (`SKILL.md`, § Parallel Safety Rules): if the re-dispatched task would touch a file an in-flight sibling task touches, the re-dispatch is **sequenced** (waits for the sibling) rather than parallelized. The `work-verifier` already checks worktree state and file conflicts between parallel tasks as part of its procedure (`work-verifier.md`, § Parallel tasks that were in-progress and § Worktree tasks); the orchestrator consumes that finding rather than re-deriving it. The re-dispatch is bound by the same parallel-safety floor every dispatch is — no new rule is introduced.

**Mode-conditional surfacing.** In **autonomous mode (`--autonomous`)**, the diagnose-and-recover sequence proceeds automatically (no human is watching), with each action logged to `adaptations`; the *escalation* leg of the reused retry rule (`SKILL.md`, § Failure Handling — "then escalate") still stops for the user, consistent with the Autonomy table's stop conditions (`SKILL.md`, § Autonomy Modes). In **interactive or supervised mode**, before re-dispatching a confirmed orphan the orchestrator surfaces a one-line note such as: "Task #N's agent appears orphaned (sustained OVERRUN; work-verifier confirms no live agent); re-dispatching per the timeout/retry rule." The one-line note rides the existing check-in cadence (`dispatch-policy.md`'s Interaction with Health Monitoring section) and does not introduce a new checkpoint. The **read-only diagnosis** (the `work-verifier` dispatch) requires no pre-approval in any mode.
...
**No-beat fallback (nothing in flight).** The three resolutions above all fire at a beat, and a beat requires either an in-flight dispatch to return or a user message to arrive (`dispatch-policy.md`'s Interaction with Health Monitoring section). On a single-chain run in autonomous mode, neither is assured: the escalating task is not itself in flight — it already failed, which is why the gate fired — no other task may be `in_progress` to return and produce a beat, and unattended operation gives no guarantee a user message ever arrives. When the orchestrator writes `pending_fable_confirm` and finds zero tasks `in_progress` at that moment, it does not leave the record waiting on a beat that may never come: it applies the default-NO resolution immediately, in the same turn — clear `pending_fable_confirm`, run the 3rd attempt on the recorded `opus` model, and continue the run. This is the same default-NO outcome the deadline-passed bullet above describes, reached without waiting for a beat to trigger it.
...
Two path-exclusion rules apply at this evaluation point. First, the **Step 4 — Process results** outcome table's **Scope issue** row ("Scope issue — agent says the plan is wrong or incomplete → Pause chain. Ask the user whether to re-plan or adjust.") pre-empts this sub-step for the same stage: a paused chain never reaches a clean stage boundary, so the two never both fire for a single stage. Second, if two or more remaining tasks are now-redundant (rather than impossible-as-written or falsely-assumed), those tasks must route through the scope-drop gate (user approval) rather than being silently removed — redundancy-removal is not laundered past the reflection beat's escalate-as-reduction branch.

**Boundary guard.** The re-plan fires only at a clean stage boundary: all tasks in the finished stage have `status: completed`, no parallel-dispatch window is open, and no task is `in_progress`. When a parallel window is still open (one or more tasks are still running), the re-plan waits — it does not interrupt in-flight work. Tasks that are `in_progress`, already `failed`, or in an open parallel window are never revision candidates; only `pending` and `blocked` tasks are eligible. This uses the same stage-boundary point the reflection beat uses — not a new boundary. Additionally, the re-plan trigger is suppressed when the run is wrapped in `ralph` (see the SKILL.md Ralph Loop Integration section): in that mode, ralph owns per-iteration re-planning and the team manager must not interpose a mid-run re-plan. **Dispatch freeze while a rewrite is in flight.** From the moment a planner is spawned to rewrite the board — whether by this sub-step's re-plan, or by the task-too-large split (`SKILL.md`, § Failure Handling, the "Task too large" row: pause the task, dispatch a planner to break it into subtasks, replace the original) — until that rewrite completes or is abandoned, the dispatch loop spawns no new work: Step 2 does not advance. The orchestrator still answers user messages and still runs its scheduled beats — the freeze stops dispatch, not conversation.
...
**Board-rewrite invariants.** On a critic-ACCEPTED, non-scope-dropping revision, the board rewrite proceeds subject to five invariants: (a) only `pending` and `blocked` tasks are revision candidates — no other status is touched; (b) `completed`, `failed`, and `in_progress` tasks are immutable; (c) the run-id, state file, and `plan_file` are retained — no new run, no new plan document; (d) `blocked_by` chains remain valid after rewrite — no orphaned or dangling edges; (e) for every revised task, the stored `description_ref` anchor is re-derived from the task's (possibly new) heading slug in the updated `plan_file`, so the Phase 3 Step 3 heading-grep (`state-schema.md`'s `description_ref` (plan-doc pointer mode) section) resolves — no `pending` or `blocked` task is left with a `description_ref` whose heading is absent from the updated plan document. The board write is ordered last in the transition so that an interrupted rewrite never points the board at headings the plan document lacks.
