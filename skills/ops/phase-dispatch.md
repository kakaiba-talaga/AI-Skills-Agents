...
**Status–spawn atomicity (Non-negotiable #11).** Item 1 (the `in_progress` state-file `Edit`) and items 4–5 (the `Agent()` spawn[s]) MUST be emitted in the **same assistant message** — never split across turns. Do not write `in_progress`, and do not report a task as running in a dashboard or in prose, in any message that does not also contain its spawn. For a parallel batch, the single `Edit` plus all the batch's `Agent()` calls ride one message. A status written without a spawn beside it is a phantom `in_progress` that `resume` and `status` will later treat as a live-but-orphaned dispatch. Re-displaying an already-running task in a later turn is not a new transition — that is fine. (Mechanically: emit the state-file `Edit` and the Agent tool call(s) as parallel tool uses within one assistant message.)
