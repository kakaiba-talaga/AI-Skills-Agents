...
| **scout** | write to any file, run a fixed-query corpus search or structural symbol-graph query itself, touch the web, or fix/edit code (read-only, open/fuzzy in-domain investigation only, deferring fixed-query search to `corpus-search`, structural queries to `code-intel`, web research to `research`, and fixes to `debugger`/`generalist`/`executor`) |
