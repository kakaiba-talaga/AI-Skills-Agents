...
Before continuing into the `Spec or requirement text`, `execute`, or `trivial` routes above (the three that lead to a new run), run the five stale-artifact detections defined in `handoffs.md` § Handoff cleanup: stale handoff subdirectories (item 4), stale cleanup records (item 5), stale terminal boards (item 6), abandoned non-terminal boards (item 7), and unreadable boards (item 8). The `trivial` route runs this check as the first thing it does in Trivial Dispatch below, before its own board exists, so the scan never has to special-case the current run. Skip this step for `resume`, `status`, and `save`, which continue an existing run rather than starting one. Skip it entirely when neither `.ops-state/` nor `.agents/handoffs/` exists yet, since there is nothing on disk to check. Each detection warns and asks per its own phrasing; none of the five auto-deletes, and the five render one batched prompt between them.
...
**ClickUp context enrichment:** If a ClickUp task ID is referenced in the spec, pull task details before planning. Every ClickUp action routes through the `/clickup` skill — see the ClickUp Actions rule in the deployed global instructions (`CLAUDE.md` for Claude Code, `.cursor/rules/clickup-actions.mdc` for Cursor). There is no team-manager direct-call exception for this lookup, read-only or not.

Run a readiness precheck first, using only local checks that never touch the ClickUp API and so are not themselves a ClickUp action: confirm `jq` is installed, confirm `~/.claude/config/clickup/config.json` exists, and confirm — by field presence only, never by reading the token value — that the token and workspace this lookup would use are set: the named `--token`/`--workspace` if the spec names one, otherwise `default_token`/`default_workspace` (e.g. `jq '.default_token, .default_workspace' ~/.claude/config/clickup/config.json` to confirm both resolve to something, never the token's own value). If any of these three is missing, skip enrichment, print a one-line notice ("ClickUp enrichment skipped: <reason>"), and proceed to plan-clarity evaluation without asking the user anything — enrichment never blocks the run. The precheck exists precisely so the skill's own first-run prompts (an offer to install `jq`, an offer to migrate a pre-v1.5 Cursor config, first-run token setup) never fire mid-Phase-1: each of those three prompts is gated on exactly the condition this precheck already screened for.

When the precheck passes, invoke the `/clickup` skill via the Skill tool for a single, combined, read-only request: the task's details and its comments (e.g. `/clickup --task <id> --format json`, asking for the comments in the same invocation rather than a second call). Only that combined lookup is in bounds. Everything else is out of bounds:
...
Skip on the same terms after invocation: any error the skill reports, an empty result, or a rate-limit response skips immediately with no retry — do not apply the ClickUp skill's own rate-limit backoff policy here; enrichment is advisory, runs at most once, and is not worth spending Phase 1 time retrying. Whatever the failure: never retry with different credentials, never re-resolve a token, never prompt the user for one, and never offer to set one up.

**This invocation costs a turn.** `/clickup` is a nested skill invoked via the Skill tool, so it falls under the Skill-tool mechanism limitation described in `SKILL.md` Non-negotiable #10: the assistant's response for that turn IS the skill's output, and Phase 1 cannot continue in the same turn. Phase 2 has not created the run's state file yet at this point, so the `pending_nested_skill` write-before/clear-after persistence in Non-negotiable #10 has nothing on disk to write to and does not apply here — only the turn boundary itself does. End the skill's response with a one-line note that Phase 1 resumes on the next message, then continue planning there using the retrieved context; this is an ordinary next message, not a formal `/ops resume`, since no state file exists yet for that command to read. In plain terms: referencing a ClickUp task ID in the spec now costs the user one extra round trip before planning starts.

Extract title, description, status, checklist items, and comments from the skill's output, and attach them verbatim into the planner's or interviewer's brief under `## Context`. Phase 1 then continues into plan-clarity evaluation exactly as before.
