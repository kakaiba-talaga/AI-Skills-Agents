...
**Phase 4's own dispatches are enrolled like any other.** Creating a deliverable, running the final verifier, and relocating documentation each get an internal bookkeeping task (see `SKILL.md`, Internal Tasks) that transitions to `in_progress` in the message that spawns the agent. Reaching Phase 4 does not suspend that requirement. It lapses only once step 9b has deleted the board, because there is then no file to write.

**The step 7a ledger capture is bookkeeping, not a dispatch.** It writes its file directly and emits no spawn tool call, so no board row can honestly describe it as running. It still gets an internal row, recorded retroactively as `completed` once the write has landed, and never transitioned through `in_progress`. That is the treatment the deslop pass row already gets on the `Skill()` path (see the two-kinds-of-state-file-writes rule in `phase-dispatch.md`, and step 4 of the deslop integration in `integrations.md`), and for the same reason: a status written where nothing was spawned is the phantom `in_progress` that Non-negotiable #11 forbids. Which of the two treatments applies is settled by reading back whether the turn emitted a spawn tool call, never by which step of the phase is running.

**And Phase 4 closes them out like any other.** When one of those dispatched agents returns, record `completed_at` and `duration_seconds` on its internal task and set the task's terminal status, following the same procedure a pipeline task's return follows (see Phase 3 Step 4 in `phase-dispatch.md` for the fresh read, the targeted `Edit`, the read-back verification, and the rule that `duration_seconds` comes from the completion notification rather than from subtracting timestamps). Enrolment without this counterpart wedges the phase: the zero-in-progress checks guarding steps 4, 9, and 9b all read the board these internal tasks live on, so one left sitting at `in_progress` is a guard that never clears, and the guard before 9b is the last point at which the check is evaluable at all. None of this is particular to Phase 4: the general form, which every site that enrols a dispatch follows, is in `orchestrator-obligations.md` § Closing out what you enrolled.
...
> **Commit gate, immediately before step 9:** check whether the working tree still holds changes this run made. If it does, a task's diff never reached a commit. Enrol a commit task now and dispatch it, while the board is still on disk for the dispatch to ride, then let the guard below wait for it. This is the last point where that is possible: step 9b deletes the board, and the window after it belongs to the completion menu's own dispatches. See `orchestrator-obligations.md`.

> **Re-entrant guard, immediately before step 9:** re-run the zero-in-progress check once more — read the state file and verify no tasks are `"in_progress"` before step 9's sweep begins. This confirms that nothing steps 1 through 8 put in flight (the deliverable dispatch in step 2, the verifier in step 3) is still running as cleanup starts. Step 7a puts nothing in flight: it spawns no agent, so it never writes `in_progress` and has no row for this guard to wait on. It does not, and cannot, cover work that step 9a itself puts in flight after this point. See the guard immediately before step 9b for that.
...
9. **Clean up ephemeral (short-lived; this run only) temp files, handoffs, state, and advisory preflight run artifacts.** This runs as three ordered sub-steps: 9a sweeps for anything durable and relocates it, 9b then deletes unconditionally, and 9c verifies the deletes landed and renders the Cleanup block.
   ...
   **9a. Durable-content relocation sweep (runs before any delete).** Before any delete, sweep this run's ephemeral scaffolding (the board file, the save file if one exists, and this run's handoff files) for content that outlives the run, and route each item to its real home. An item already captured through its normal channel earlier in the run (a handoff written at its stage transition, the step 4a timing write, or the step 7a ledger record) does not need a second relocation here; this sweep exists to catch what a normal channel would otherwise miss. Any dispatch or nested-skill invocation the sweep itself makes to relocate content, such as the documentor dispatch below for non-obvious decisions, runs in the **foreground** and must return before 9b begins: the relocating agent may well be reading the very board or handoff files 9b is about to delete.
   ...
   | Unresolved findings and anything under a handoff's `### Open items` heading | Surfaced explicitly in the relocation report (9c), so the user sees it without reading a deleted file. An item already carded during the run is surfaced by subject and disposition, carried into the report as text, never as a reference to a board that 9b is about to delete |
   | Non-obvious decisions made mid-run that later work depends on | Documentation (dispatch `documentor`) or the plan doc |
   | Investigation or forensic evidence that explains why a change took the shape it did | The documentation for the change |
