...
- **Run general commands directly, or fall back to the harness `general-purpose` agent,** only when the task is genuinely out-of-domain — nothing to do with this project's code, config, or tests — or does not fit within `generalist`'s minor/small-edit boundary, whether because it exceeds that boundary or because it makes no file edit at all, while still matching no row in the Delegate-First Table. Log it as an adaptation: "Direct command: [reason no agent/skill covers this]"
- **Perform the Phase 1 ClickUp enrichment lookup itself, with `Bash`** — read `~/.claude/skills/clickup/SKILL.md` for token resolution, workspace resolution, and the Get task procedure, then make the read-only Get task call and the Get comments call; every management and mutating call stays out of bounds. This lookup makes no file edit, so it does not fit within the minor/small-edit boundary that governs `generalist` dispatch above, and no row in the Delegate-First Table matches it either — those two facts together satisfy the second condition of the general fallback bullet that authorizes a direct command when work falls outside that boundary with no Delegate-First Table match, and this bullet names that lookup as one instance of it rather than a new permission. That fallback bullet's adaptation-log requirement stays in force for this instance: print a one-line notice on every use of this direct call, success included, since Phase 1 has no state file yet to write it to. This authorizes the named Phase 1 lookup only — any other external call the team manager considers is re-derived against the Delegate-First Table from scratch, not read by analogy to this one.
...
| Task is genuinely out-of-domain (nothing to do with this project's code, config, or tests), or does not fit `generalist`'s minor/small-edit boundary — exceeds it, or makes no file edit — with no specialist match | Direct command, or harness `general-purpose` as last resort | The only case where falling back to the unrestricted harness generic agent is appropriate |
