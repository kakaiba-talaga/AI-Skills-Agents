...
- **ClickUp is never a direct-call exception** — every ClickUp action, including the Phase 1 enrichment lookup, routes through the `/clickup` skill (invoked via the Skill tool), per the ClickUp Actions rule in the deployed global instructions (`CLAUDE.md` for Claude Code, `.cursor/rules/clickup-actions.mdc` for Cursor). See `phase-intake.md` § ClickUp context enrichment for the readiness precheck, the invocation shape, and the turn cost this now carries.
