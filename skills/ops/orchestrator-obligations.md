...
## Verifying what you report yourself

The rule above checks a claim that arrived from somewhere else. Nothing checks the ones that start with you. An agent's claim at least has a sender to distrust, and there is a ritual for that; your own characterisation of a file arrives with no sender at all, and it gets repeated for as long as nobody asks where it came from.

So before asserting a property of an artifact, name the observation that established it. The trap is that a story about how the thing came to be feels exactly like a reading of what it contains. Watching a file get produced, or interrupted, or generated by a step whose shape you know, tells you what happened to it and nothing about what it says. Provenance is not content. If the honest answer to "what established this" is an inference from circumstance, you have not looked, and the assertion is not yet yours to make.

Two of these are on record, and neither was a reading that went stale. An orchestrator killed an agent mid-run, inferred from the interruption that the output must be partial, and called the work half-done for nine hours across three separate turns, recommending a revert each time. The first read of the file came after the third recommendation, and only because the user asked what unfinished work was being referred to. It was a complete implementation, three hundred and fifty-odd inserted lines, wired end to end and missing only its tests. The other asserted in a table, as fact, that a particular documentation line was tracked by a build-transform anchor and could not be edited safely. What had been observed was that two generated files differed on that line. The anchor was inferred from the difference, and it did not exist. Both had seen something next to the claim and spent it as though it were the claim.

The general rule is cheap, because the honest answer is usually one command. Where it stops being optional is a recommendation to revert, delete, or skip: that recommendation cites a command run in the same turn against the thing being discarded, and "I was there when it was made" is not that command. A one-second `git diff --stat` on the file would have ended those nine hours, because the insertion count alone is incompatible with half-done. This binds what you recommend to the user. It is not about the recovery path a failure verdict already governs, where the revert is the verdict's and the evidence is the verifier's.

Note how this differs from a reading that went stale. Staleness has a true observation behind it and a file that moved since, so re-reading fixes it. This has no observation behind it at all, so there is nothing to re-read and no source whose authorship you could check. The question that catches it is not "is this still true" but "how do I know this", and that one is answerable in a line or not at all.
