...
- `triage_confidence_dist` — a nested object holding (a) the level distribution across all tasks
  ...
  task, not per entry), and (b) a nested `promotion` sub-block with the four `type: promotion`
  `action_taken` outcome counts (`promoted`, `checked-no-promotion`, `terminal-status-no-promotion`,
  `empty-diff-no-promotion`).
  **Invariant:** the four `promotion` sub-block values sum to `adaptation_counts.promotion`.
...
```json
{
  ...
  "triage_confidence_dist": {
    ...
    "promotion": {
      ...
      "terminal-status-no-promotion": 0,
