...
> **Reference:** You MUST Read `~/.claude/skills/cross-memory/subcommand-init.md` for the full init flow (Steps 1–5 — provisioning, adapter detection, reachability probe, always-on filter + sentinel write, reflect-staleness check, summary output in human / JSON / verbose), the per-harness behavior tables, idempotency contract, the "what init does NOT do" boundary list, and the reuse-of-doctor's-check-primitives section. If the file is missing, proceed using the inline summary: init delegates to the lazy-provisioning sequence in `## Config`, runs the adapter-selection chain, probes the harness-native `MEMORY.md` for reachability, invokes the always-on tier filter and injection-block formatter, dispatches `active_adapter.update_sentinel_block(content)` (no-op on cursor/generic at v1), reads the per-project `state.toml` to fire the reflect-staleness hint when `delta_days > reflect_staleness_threshold_days`, and emits a one-line summary (or schema-version-1 JSON under `--json`). Init is additive only — never deletes, overwrites, or repairs.

---

## Subcommand: doctor

> **Reference:** You MUST Read `~/.claude/skills/cross-memory/subcommand-doctor.md` for the full doctor reference: the four operating modes (default / `--pre-deploy` / `--post-deploy` / `--check`), the check-group definitions (Groups A–E with finding shapes and PASS/WARN/FAIL criteria), per-harness applicability for sentinel checks and `sc-behavioral-walk`, the post-deploy walks for Claude Code and Cursor, output formats and exit codes, verdict aggregation rules, and the reflect-staleness hint that fires after the overall-verdict line. If the file is missing, proceed using the inline summary: doctor is read-only, dispatches no agent, runs check primitives whose stable identifiers are defined in `## Check-name vocabulary`, and emits PASS/WARN/FAIL/NOT-APPLICABLE per section plus an aggregated overall verdict; exit codes apply under `--json` only.

---

## Subcommand: reflect

> **Reference:** You MUST Read `~/.claude/skills/cross-memory/subcommand-reflect.md` for the full reflect reference: the eight-step routing flow, the four source pipelines (Source 1 Claude-Code-only session transcripts, Source 3 git + working tree, Source 4 plan docs + handoffs + dispatch logs, Source 5 explicit `--from`), the interactive command loop (`save` / `decline` / `edit` / `done`), supersede flagging, the deterministic anti-redundancy filter (three signals, four reference sets, tokenization rule, disjunctive trip rule, filter-reason tags, `--verbose` decisions), and the cross-harness applicability matrix. If the file is missing, proceed using the inline summary: reflect dispatches the `cross-memory` agent with `intent: distill`, the skill stages Source 3 to `_tmp_reflect_source3_<ts>.md` and passes Source 4 globs through brief constraint fields, the agent applies the OR-of-three deterministic filter against canonical + archive + decline-ledger reference sets plus the LLM-prompt-applied exclusion corpus from `~/.claude/CLAUDE.md`'s "What NOT to save in memory" section, and the skill writes `state.toml.reflect.last_reflect_at` on `done`.
