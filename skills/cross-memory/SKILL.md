...
- **`reflect_declined.md` ledger**: created lazily at `~/.cross-memory/projects/<slug>/reflect_declined.md` on first decline; consulted by the agent as Reference Set C.
- **`state.toml` per-project state file**: `~/.cross-memory/projects/<slug>/state.toml`; `reflect.last_reflect_at` field read by the staleness hint in `## Subcommand: doctor`.
- **`reflect_staleness_threshold_days` config field**: `## Config` above.
- **Adapter selection chain**: `## Adapter selection` above.
- **Staleness hint fired by `doctor` and `init`**: `## Subcommand: doctor → Reflect staleness hint` above.
