...
**Subagent brief grammar:** every inline subagent prompt must contain these four sections, even when contents are brief. Empty sections create improvisation risk; explicit short sections do not. The four sections are:

- `## Task` — one imperative sentence stating what the subagent must produce.
- `## Scope` — explicit list of file paths and modules the subagent may read or modify. Use absolute or repo-relative paths.
- `## Acceptance Criteria` — numbered list of testable assertions the subagent's output must satisfy. Copy these verbatim from the phase plan document's Tasks section when applicable.
- `## Constraints` — the constraint block from "**Constraints for all subagents**" below, plus any task-specific lane boundaries (e.g., file-class allowlist for the implementor — see below).

This grammar is self-contained for `next.md` — it does not depend on any external contract file. The grammar is the contract for this command.
...
**File-class allowlist for the implementor subagent** (include verbatim in every implementor prompt, in addition to the constraints above):

The implementor may read any file in the project, but may modify files only within these classes:

- **Source files** — application code in the project's source tree (e.g., `src/**`, `lib/**`, language-appropriate locations).
- **Test files** — test code colocated with source or in the project's test tree (e.g., `tests/**`, `**/__tests__/**`, `*.test.*`, `*.spec.*`).
- **Config files** — project configuration genuinely required for the task (e.g., `package.json`, `pyproject.toml`, `Cargo.toml`, language-specific lockfiles when adding/removing dependencies, build configs when changing build behavior).

The implementor MUST NOT modify these classes:

- **Plan documents** — anything in `plan/` (e.g., `plan/INDEX.md`, `plan/QUEUE.md`, `plan/BLOCKERS.md`, `plan/<NN>-<name>.md`). Plan documents are coordinator-managed; the implementor flags any plan-doc concern in the handoff but never edits the file directly.
- **Project-scoped agent or command definitions** — anything in `.claude/` or `.cursor/` (e.g., `.claude/commands/*.md`, `.claude/agents/*.md` if present). These are infrastructure, not implementation surface.
- **Other workflow markers** — files generated by `/next` itself or by other coordinator tooling, even when they live outside `plan/`.

If the task genuinely requires modifying a file outside the implementor's allowlist, the implementor must report the requirement in the handoff with the file path and the reason, and stop. The coordinator decides whether to escalate the scope (and may dispatch a different role for that work).
