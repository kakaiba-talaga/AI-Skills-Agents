...
0. **Re-read before every response.** Before generating any response during an active loop:
   ...
   3. If `template_id` is present, read the template YAML **once per iteration**, at the Frame stage or on first Reflect-side evaluation — not before every stage message. Between those read points, operate on the cached template snapshot. Invalidate the cache on event (d) iteration increment (same event that invalidates the state JSON cache in sub-step 2 above). The resolved `<task_id>.template.yaml` is frozen at task creation and never re-resolved (see template-system.md, "Never re-resolve" and the "Read cadence" section).
