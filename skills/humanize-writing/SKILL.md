...
**`Humanize Writing`** appears on the **opening line** of each assistant turn only. Do **not** prefix every bullet or heading in the same turn.

The **first line** of each assistant turn for this command MUST begin with: **`Humanize Writing`**

Apply the badge on the opening line for: the voice-selection question (when asked), rewrite-in-progress notes, and the final rewritten text.

**Format:** **`Humanize Writing`** (bold backtick-wrapped) as the **first element** on the **opening line** of the turn.
