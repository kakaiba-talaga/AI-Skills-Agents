...
| 4 | `task-verify-json-schema-fixtures` | Walk 4-6 mock briefs (including 1 JSON-with-prose-noise collision case) through the agent body's `## Brief Format` prose and confirm refusal posture (addresses scoper G1 / C-PLAN-6 and the brief-format precedence rule) | verify | verifier | `agents/code-intel.md` (read-only) | 25 | task-verify-r7-sections |
...
**Description.** Verifier composes 4-6 mock briefs and walks each one through the agent body's `## Brief Format` section to confirm the documented refusal posture matches what would happen at runtime (addresses scoper G1 / C-PLAN-6). This is a **documented walkthrough**, not an executed test. The agent body's prose is the source of truth, and the verifier's job is to confirm that prose says the right thing for each fixture. The fixture set covers, at minimum: (a) a valid brief that should be accepted, (b) a brief with an unknown field that should be refused per `additionalProperties: false`, (c) a brief with a missing required field (`query_type` or `symbol`) that should be refused, (d) a brief with malformed JSON syntax that should be refused, (e) a brief with a value that exceeds R9's hard caps (e.g., `max_files: 10000`) that should be refused per the override-cap rule, (f) a JSON-with-prose-noise collision case (see new acceptance bullet below).

**Acceptance criteria.**
- Verifier composes the 4-6 fixture briefs as Markdown blocks in the report.
...
- At least one fixture must exercise the JSON-with-prose-noise collision case (a labeled-prose brief that contains an example ` ```json ``` ` fenced block, AND/OR a JSON brief preceded by hand-written prose containing the literal token `Query:`). Verifier confirms the agent body specifies that the JSON-fenced block wins as the authoritative brief and the labeled-prose lines are ignored. The fixture's stated disposition must be **accept the JSON-fenced block as authoritative; ignore the surrounding `Query:` lines**. This fixture is regression coverage for the brief-format precedence rule added in `## Brief Format`.
- Verdict: **PASS** | **REVISE** | **REJECT**.

**Risks / notes.** This is a behavioral fixture verification, not an executed test. The pattern matches `task-nested-skill-integration-spec` — produce a documented walkthrough that a human can read end-to-end. Parallel-safe with the other M5 verifier tasks because each touches `agents/code-intel.md` read-only. The collision-case fixture was added during the critic-revision pass as the regression test for the BLOCKER B1 fix in `agents/code-intel.md`.
