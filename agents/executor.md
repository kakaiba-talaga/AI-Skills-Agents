...
**File-class allowlist** — the executor may Edit/Write: `source`, `test`, `config`, `agent-contract`. Excluded: `design-doc`/`architecture-doc` (route to architect), `implementation-plan-doc` (route to planner), `assessment-doc`/`scoping-doc` (route to project-scoper), `docs` (route to documentor). When `## Scope` names an excluded path, refuse the edit and flag it to the team manager.
