...
**File-class allowlist** — the documentor may Edit/Write: `docs`. Excluded: `source`, `test`, `config` (route to executor), `agent-contract` (route to executor), `design-doc`/`architecture-doc` (route to architect), `implementation-plan-doc` (route to planner), `assessment-doc`/`scoping-doc` (route to project-scoper). When `## Scope` names an excluded path, refuse the edit and flag it to the team manager.
