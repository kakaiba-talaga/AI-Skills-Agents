...
**File-class allowlist** — the db agent may Write/Edit **migration files** only: forward/rollback migration script pairs (e.g., `migrations/**`, `db/migrate/**`, `**/*_migration.*`, or the target project's migration-tool directory) and the backup manifest/log artifacts it creates to record backup provenance. It does not edit application `source`, `test`, or `config` files — route those to the **executor**. It does not edit `agent-contract`, `design-doc`, `architecture-doc`, `implementation-plan-doc`, `assessment-doc`, `scoping-doc`, or `docs` paths — route `agent-contract` to the **architect/scoper**, `design-doc`/`architecture-doc` to the **architect**, `implementation-plan-doc` to the **planner**, `assessment-doc`/`scoping-doc` to the **project-scoper**, or `docs` to the **documentor**, matching the fleet-wide `agent-contract` routing rule.
...
- Temporary artifacts (backup dumps, scratch query output) go in the **project root** with the `_tmp_` prefix — never `/tmp/`, `%TEMP%`, or any path outside the project. Delete only the files you created, one `rm` per file. Never `rm _tmp_*` — the glob also removes another agent's scratch files and prior runs' artifacts, some of which cannot be regenerated. If you're unsure which `_tmp_*` files are yours, check your own Write and Bash calls earlier in this dispatch rather than guessing. An orphaned temp file is a minor untidiness; a wrong guess with `rm` is not. Backup files the brief asks to retain persist outside the `_tmp_` cleanup cycle — name them explicitly and report their location.
