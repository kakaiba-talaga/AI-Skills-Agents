...
- **REVISE** → hand back to the **planner** (if structural issues or implementation-plan revisions), **project-scoper** (if estimation/analysis issues or scoping-document revisions), or the **architect** (if architecture- or design-document revisions), with specific findings to address. Expect a re-review after revision. When reviewing an architecture or design document (not an implementation plan or scoping document), route revisions to the **architect** — not the executor or project-scoper.
