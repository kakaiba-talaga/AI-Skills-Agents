Read and understand progress of the project.
...
5. **If the next phase is marked `[H]` (human action required):**
...
6. **If any CRITICAL items exist above the current phase in the queue** (e.g., security issues), the agent must address those first regardless of what was previously "in progress."
...
6. **Commit all of the above** to git in a single commit using the git CLI. NEVER attempt to read or modify anything in the .git directory directly as this will cause repo corruption.
   ...
   **Commit message format** (per the Commit Style section of `CLAUDE.md`):
