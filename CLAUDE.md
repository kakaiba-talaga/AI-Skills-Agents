...
| `skills/ops/orchestrator-obligations.md` | `skills/ops/SKILL.md` (Non-negotiable #13 and the companion index), `skills/ops/phase-dispatch.md` (Step 4's disposition of a finding an agent reported but did not fix, and the Phase 2.5 preflight-validation paragraph enrolling that dispatch), `skills/ops/phase-completion.md` (Phase 4's own dispatches enrolling like any other, and the 9a relocation-sweep row for unresolved findings), `skills/ops/handoffs.md` (the downstream-briefs section on carrying a finding forward without dispositioning it), `skills/ops/state-schema.md` (the `adaptations` array's `type: finding` enumeration), `skills/ops/phase-intake.md` (the close-out note beneath the routing table, and the two trivial-route clauses citing it: the promotion check's change-analyzer dispatch and the deferred branch-creation dispatch), `skills/ops/integrations.md` (the deslop path's enrolment paragraph covering its three spawns), `skills/ops/dispatch-policy.md` (the foreground rule for a dispatch whose result the route consumes in the same turn, which the close-out-before-stop argument rests on) |
