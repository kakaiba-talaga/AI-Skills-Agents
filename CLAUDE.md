...
| `skills/ops/tool-restrictions.md` | `agents/generalist.md` (§ Minor/small-edit boundary, summarized in the Delegate-First Table row and Subagent Dispatch Decision Framework rows), `agents/infra.md` and `agents/db.md` (each's § Model Escalation Policy, summarized in § Model Escalation for `infra`/`db`), `agents/scout.md` and `agents/docs-lookup.md` (dispatch criteria summarized in the Delegate-First Table row and Subagent Dispatch Decision Framework row), `skills/ops/phase-intake.md` (ClickUp enrichment step's citation of the "What the Team Manager MAY Do Directly" list), `skills/ops/dispatch-log.md` (Subagent Dispatch Decision Framework audit description and procedure), `skills/ops/SKILL.md` (companion index table entry + § Team manager tool restrictions reference), `tooling/transform-cursor-ops.{ps1,sh}` (Cursor-path patch of that same reference) |
...
| `skills/ops/brief-contract.md` | `skills/ops/SKILL.md` (Agent Briefing Format), `agents/{executor,verifier,debugger,git-master,project-scoper}.md` (`## Brief Format` subsections), `agents/_shared/brief-format-snippet.md` (defaults list, incl. Premise Accuracy pointer) |
...
| `skills/ops/SKILL.cursor.additions.md` | `tooling/transform-cursor-ops.{ps1,sh}` (executable source of truth; script wins on disagreement), `skills/ops/SKILL.md` and `skills/ops/phase-*.md` companions (anchors quote this text; go stale when either changes); hand-maintained, no generator, excluded from every target in `tooling/deploy-manifest.json` |
...
| `skills/ops/state-schema.md` (§ Cleanup record file) | `skills/ops/phase-completion.md` (writes the record during the relocation sweep, deletes it once its readers finish, excludes it by name from step 9b's derived delete enumeration), `skills/ops/completion-options.md` (reads `worktrees_created` from it for the worktree-cleanup-by-provenance check), `skills/ops/handoffs.md` (Phase 1 stale-artifact detection for an orphaned record) |
| `skills/ops/state-schema.md` (§ Anchor derivation for targeted `Edit` and § Status enum) | `skills/ops/phase-dispatch.md` (Step 1's fresh-read requirement cites the anchor-derivation section; Step 4's outcome table cites the status enum), `skills/ops/handoffs.md` (item 6's stale-terminal-board check cites the status enum to require every task be terminal before treating a board as abandoned; item 7's abandoned-non-terminal-board check cites the same enum for the converse case; both now depend on an unrecognized value counting as non-terminal) |
| `skills/ops/handoffs.md` (§ Handoff cleanup) | `skills/ops/phase-intake.md` (the stale-artifact detection step and the Trivial Dispatch note, both of which cite the items by number and by count), `skills/ops/phase-completion.md` (step 9a's relocation sweep, which items 6, 7 and 8 re-run against a stale board), `skills/ops/state-schema.md` (§ Status enum, § Cleanup record file, and the `.ops-state/` naming rule) |
| `skills/ops/dispatch-policy.md` (canonical dispatch-mode rule) and `skills/ops/timing-edge-cases.md` (canonical timing edge-case rules and their count) | `skills/ops/phase-dispatch.md` (dispatch-loop wiring, liveness gate, nested-skill write-before/clear-after ritual and skill-call bookkeeping, in-flight accounting rule), `skills/ops/interruption-recovery.md` (cancel, pause, and resume behavior), `skills/ops/phase-intake.md` (`resume` row's orphan test), `skills/ops/phase-completion.md` (completion phase's re-entrant guarding; also carries two of the three timing-rule-count restatements), `skills/ops/completion-options.md` (completion menu's dispatch mode), `skills/ops/integrations.md` (cleanup-skill entry gate and its re-verification dispatch mode), `skills/ops/subcommand-save.md` (save path's nested-skill entry gate), `skills/ops/orchestrator-obligations.md` (close-out-before-stop argument, which rests on the same-turn foreground rule), `agents/work-verifier.md` (Handoff verdict table's liveness qualifier), `skills/ops/README.md` (third timing-rule-count restatement) |
| `skills/ops/orchestrator-obligations.md` | `skills/ops/SKILL.md` (Non-negotiable #13, the Reference blurb under Non-negotiable #14, and the companion index), `skills/ops/phase-dispatch.md` (Step 4's disposition of a finding an agent reported but did not fix, the Phase 2.5 preflight-validation paragraph enrolling that dispatch, and the passing-outcome row's commit trigger), `skills/ops/phase-completion.md` (Phase 4's own dispatches enrolling like any other, the 9a relocation-sweep row for unresolved findings, and the commit gate immediately before step 9), `skills/ops/completion-options.md` (the menu's commit precondition note), `skills/ops/handoffs.md` (the downstream-briefs section on carrying a finding forward without dispositioning it), `skills/ops/state-schema.md` (the `adaptations` array's `type: finding` enumeration), `skills/ops/phase-intake.md` (the close-out note beneath the routing table, and the three trivial-route clauses citing it: the promotion check's change-analyzer dispatch, the deferred branch-creation dispatch, and the On-result commit clause), `skills/ops/integrations.md` (the deslop path's enrolment paragraph covering its three spawns), `skills/ops/dispatch-policy.md` (the foreground rule for a dispatch whose result the route consumes in the same turn, which the close-out-before-stop argument rests on), `skills/ops/README.md` (the companion-table row summarising this companion) |
