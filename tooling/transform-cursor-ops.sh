...
# ---------------------------------------------------------------------------
# PATCH 57 — Mode-aware loop cap: 3rd loop no longer escalates a model tier
# ---------------------------------------------------------------------------
rep(
    "the 3rd loop escalates the model one tier (see Model Escalation for tiers, the `opus → fable` confirmation gate, the at-ceiling short-circuit, and the security-reviewer opus ceiling). In autonomous mode, loops 4 and 5 re-run on the already-escalated model (debugger findings from the 2nd loop carried forward) — the model tier does not climb further and the fable gate does not re-fire. Autonomous extended loops never escalate to fable silently; they run on the post-gate model (opus when the gate defaulted NO).",
    "the 3rd loop dispatches a debugger or debugger-build for diagnosis and re-briefs with the findings, rather than escalating the model (see Retry strategy and Cursor-Specific Limitations — Cursor has no model escalation, so this diagnostic re-brief takes its place). In autonomous mode, loops 4 and 5 re-dispatch with the debugger findings from the 3rd loop carried forward before escalating to the user at the 5-loop cap.",
)

# ---------------------------------------------------------------------------
# PATCH 58 — Non-negotiable #14: drop the fable-confirm gate from the list of
# legitimate reasons a turn ends without a dispatch (Cursor has no model
# escalation, so there is no fable-confirm gate to wait on).
# ---------------------------------------------------------------------------
rep(
    "an escalation is open (the failure cap, a blocker, a scope issue, the `fable`-confirm gate);",
    "an escalation is open (the failure cap, a blocker, a scope issue);",
)

# ---------------------------------------------------------------------------
# PATCH 59 — Autonomous stopping conditions: drop the fable-confirm-gate
# contrast from the at-ceiling budget escalation clause.
# ---------------------------------------------------------------------------
rep(
    "at-ceiling budget escalation (unlike the `fable`-confirm gate below, this one blocks), or all tasks terminal.",
    "at-ceiling budget escalation, or all tasks terminal.",
)

# ---------------------------------------------------------------------------
# PATCH 60 — Remove the sentence explaining the fable-confirm gate is not a
# seventh stopping condition. Cursor has no fable-confirm gate at all, so the
# distinction the sentence draws has no subject to draw it about; the
# surrounding paragraph reads cleanly without it.
# ---------------------------------------------------------------------------
rep(
    "on. The `fable`-confirm gate is one of #14's legitimate waits, not a member of this list: in autonomous mode it defaults to NO after its best-effort ~1 minute window and the run continues, so it never becomes a seventh stopping condition. Interactive",
    "on. Interactive",
)
